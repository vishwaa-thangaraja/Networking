...
void *handleClient(void *arg)
{
    ...
    if (strncmp(data->message, "SETUP", 5) == 0)
    {
        ...
        int current_ip = 1; // Fourth octet starts at 1 (0 is network address)
        for (i = 0; i < subnet_count; i++)
        {
            int start_addr = current_ip;
            int end_addr = current_ip + total_ips[i] - 1;
            char temp[200];
            sprintf(temp,"Block %d:\nStart Address: %d.%d.%d.%d\nEnd Address: %d.%d.%d.%d\nAvailable IP Addresses: %d\n\n",
                    i + 1, ip1, ip2, ip3, start_addr,
                    ip1, ip2, ip3, end_addr,
                    total_ips[i]);
            strcat(response, temp);
            current_ip += total_ips[i];
        }
        // ========================================
